...
/**
 * The per-store values that must not be state: the two debounce timers and the
 * in-flight undo snapshots.
 *
 ...
 * handle is not data, and the snapshots are whole-array copies that would be deep-watched
 * and mirrored to `localStorage` 300ms later for nothing.
 *
 ...
 * would be invisible to `endEdit`, and `flush()` would never see the pending save that
 * `_scheduleSave` set. `toRaw` reads through both that proxy and Vue's reactive one to the
 * single underlying store object, so every action agrees — and it is the same in dev, in a
 * production build, and under the tests, which is exactly what makes it trustworthy.
 */
